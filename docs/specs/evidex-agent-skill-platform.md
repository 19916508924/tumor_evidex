...
| P2     | 前端完整性与可访问性  | 拆分过大客户端组件，增加导航当前态和键盘焦点可见性，将交互热区统一到至少 44px，用设计令牌代替重复颜色常量。                                                          |
...
| 模块                      | 当前已实现                                                                                                           | 当前缺口与本轮动作                                                                                                                                          |
| ------------------------- | -------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Knowledge 前端            | 疾病、基因、变异、药物四类目录与详情；全局搜索；Evidence 和 Source 详情                                              | 保持已发布版本隔离；补真实性能预算、桌面键盘可访问性和真实交互 E2E                                                                                          |
| Ask 前端                  | 自然语言提问、可选结构化上下文、轮询恢复、反馈和引用展示                                                             | 运行阶段必须来自真实节点；停止等待需中止客户端轮询；补完整澄清、失败、恢复和引用跳转 E2E                                                                    |
| Evidence Ops 前端         | 驾驶舱、检索策略、Run、Candidate、Release、Agent/Skill/Workflow/Question Run 页面以及审核工作台雏形                  | 新增统一“发起知识更新”表单、审核队列入口和单篇 PMID 入口；修复审核字段契约、状态刷新、导航当前态和桌面可访问性                                              |
...
### 1.4 当前演示形态与桌面端约束（2026-09-17）

本节覆盖本文其他章节中已经过时的移动端抽屉和“所有匿名访问都跳转登录”要求：

- 当前产品是单用户面试演示网站，只验收 1024px 至 1920px 的桌面浏览器；Landing、公开应用与 Ops 壳均设置 1024px 最小宽度，不再维护汉堡菜单、抽屉、遮罩、焦点陷阱、移动标题栏或小屏重复入口。
- Landing、Knowledge、Ask 与 Evidence 的主导航始终单行展示“证据审核”，链接到 `/{locale}/ops/reviews`；导航本身不得横向滚动或隐藏核心入口。
- Evidex 默认就是单用户 Demo：没有 `EVIDEX_DEMO_MODE`、没有 Cookie 时，无会话用户可以直接访问完整 Ops 页面与内部接口；所有需要操作者的运行、版本和审核记录使用固定 `demo-reviewer` 身份并保留审计。已有真实会话优先；只有显式设置 `EVIDEX_DEMO_MODE=0` 时才恢复 `admin.access` 门禁。
- 演示身份只替代身份解析，不得绕过同源 `Origin`、Zod 输入、乐观锁、幂等键、必填说明、QA 阻断、状态机或发布事务。Evidex 产品页面不得显示登录、注册、用户中心、退出登录或管理员权限提示。该模式只连接隔离演示数据库，不得用于生产或共享数据环境。
- 桌面 Chrome E2E 至少覆盖 `/zh/knowledge` → 公开导航 → 审核队列 → 审核详情 → 一个不发布的审核动作。
...
- 数据不得前端硬编码；
- 指标可按本周、近 30 天切换；
- 页面只支持 1024px 以上桌面端，并在 1024px 至 1920px 保持核心状态与操作可见。
...
- 所有主导航展示当前路由状态；桌面键盘焦点清晰可见，主要交互热区不小于 44px；
...
- 桌面弹窗和二次确认支持清晰的焦点顺序、`Escape` 关闭与焦点返回；
...
5. 新证据出现在实体详情和证据详情；
6. 用户问题使用新版本并引用新证据；
7. 默认无环境变量时匿名用户可以使用固定演示审核人完成 Ops 队列、详情、草稿和非发布决定；显式设置 `EVIDEX_DEMO_MODE=0` 时匿名用户无法访问 Ops 页面或草稿接口；
8. 模型回答失败时仍展示证据卡片。
